...
# 사진 촬영 함수
last_capture_time = 0
def capture_photo(cropped_frame):
    ...
    if timestamp - last_capture_time >= 0.5:
        ...
        cv2.imwrite(filename, cropped_frame)  # 자른 이미지를 저장
...
try:
    while running:
        ...
        # 이미지 크기 자르기 (세로 방향으로 4등분 후 아래 3/4만 남기기)
        height, width, _ = frame.shape
        cropped_frame = frame[height // 4:, :]  # 아래 3/4 영역만 선택

        # OpenCV로 잘린 이미지를 화면에 표시
        cv2.imshow("Live View (Cropped)", cropped_frame)
