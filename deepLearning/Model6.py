...
# 2️⃣ 모델 생성
model = Sequential([
    Dense(512, activation='relu', input_shape=(10,), kernel_regularizer=tf.keras.regularizers.l2(0.01)),  # 첫 번째 은닉층
    BatchNormalization(),
    Dropout(0.1),
    Dense(256, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01)),  # 두 번째 은닉층
    BatchNormalization(),
    Dropout(0.1),
    Dense(128, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.01)),  # 세 번째 은닉층
    BatchNormalization(),
    Dropout(0.1),
